    ...
    for (xgroup = 0; xgroup < n_xval; xgroup++) {
        ...
        for (i = 0; i < ct.n; i++) {
            ...
            if (x_grp[i] == xgroup + 1) {
                ...
                ct.propensitytemp[k] = ct.propensity[i];
                ct.censoringProbtemp[k] = ct.censoringProb[i];
                temp += ct.wt[i];
                k++;
            }
        }
        ...
        if (split_Rule == 1) {
            ...
            (*ct_eval) (k, ct.ytemp, xtree->response_est, xtree->controlMean, xtree->treatMean,
             &(xtree->risk), ct.wtemp, ct.trtemp, ct.max_y, split_alpha, xtrain_to_est_ratio,ct.propensitytemp,ct.censoringProbtemp);
        ...
        for(i = k; i < ct.n; i++) {
            ...
            if (crossmeth == 1) {
                ...
                userH_rundown(xtree, j, cp, xpred, xtemp, k, cv_alpha, xtrain_to_est_ratio);
            } else if (crossmeth == 8) {
